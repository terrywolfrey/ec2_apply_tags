...
tags="Key=Cluster,Value=umg.sngl.cm"

###################################################  END USER SETTINGS HERE  ###################################################


# Lists all instance ID's matching the filter variable in serial text format.
id_list=$(aws ec2 describe-instances --query 'Reservations[*].Instances[*].InstanceId' --filters "Name=tag:Name,Values=${filter}" --output text | paste -d " "  -s)
